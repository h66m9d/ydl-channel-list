...
youtube-dl --yes-playlist -r 4096k -o "%(playlist_uploader)s/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s" --download-archive "../$randomdirdownloaded.txt" --ignore-errors $listurl --batch-file /tmp/list
...
tar -czvf ../$playlistuploadernamenonespace.tar.gz * ../$randomdirdownloaded.txt
cd ..
rm $randomdirdownloaded.txt
rm -r $randomdir
